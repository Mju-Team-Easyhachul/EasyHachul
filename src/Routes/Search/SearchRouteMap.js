...
class SearchRouteMap extends Component {
  constructor(props, context) {
    ...
    this.state = {
      searchLine : this.props.DijkstraTotalPath,
      searchData : 0,
      data : {
        nodes: [],
        links: [],
    ...
    this.setDijkstraRoute();
  }

  shouldComponentUpdate(){
    while(this.state.data.nodes.length != 0){
      this.state.data.nodes.pop();
    }
    while(this.state.data.links.length != 0){
      this.state.data.links.pop();
    }
    this.setDijkstraRoute();
  }
  ...
  setDijkstraRoute = () => {
    ...
    let tempData = {
      ...
      links: [
        ...
        { color: '#18A51D', type: 'STRAIGHT', source: '101', target: '123' },
    ...
    // 노선도 node 삽입
    for (var i in this.state.searchLine) {
      this.state.data.nodes.push(tempData.nodes.find(x => x.id === this.state.searchLine[i]));
    }
    
    // 노선도 link 삽입
    for (var j = 0; j < this.state.searchLine.length; j++) { 
      if (j !== this.state.searchLine.length - 1) {
        this.state.data.links.push(tempData.links.find(x => (x.source === this.state.searchLine[j] && x.target === this.state.searchLine[j + 1])));
      }
    }
